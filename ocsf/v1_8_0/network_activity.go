...
type NetworkActivity struct {
	...
	// Application Name: The network application name identified by tools such as NBAR or App ID (e.g., youtube, facebook, webex). This represents a specific network application that uses standard protocols (such as https or quic) to deliver its service.
	AppName *string `json:"app_name,omitempty" parquet:"app_name,optional"`

	// Application Protocol Name: The application-layer (Layer 7) protocol name identified by deep packet inspection or packet parsing (e.g., <code>https</code>, <code>quic</code>, <code>ssh</code>, <code>dns</code>), expressed as an IANA-registered service name from the IANA Service Name and Transport Protocol Port Number Registry.
	//
	// <p><b>Note:</b> Port numbers alone are not always a reliable indicator of the actual application protocol in use.</p>
	AppProtocolName *string `json:"app_protocol_name,omitempty" parquet:"app_protocol_name,optional"`
	...
	// Class: The event class name, as defined by class_uid value: <code>Network Activity</code>.
	...
	// Destination Endpoint: The responder of the network connection. In some contexts an event source cannot correctly identify the responder. Refer to <code>is_src_dst_assignment_known</code> for certainty.
	...
	// Source/Destination Assignment Known: <code>true</code> denotes that <code>src_endpoint</code> and <code>dst_endpoint</code> correctly identify the initiator and responder respectively. <code>false</code> denotes that the event source has arbitrarily assigned one peer to <code>src_endpoint</code> and the other to <code>dst_endpoint</code>, in other words that initiator and responder are not being asserted. This can occur, for example, when the event source is a network appliance that has not observed the initiation of a given connection. In the absence of this attribute, interpretation of the initiator and responder is implementation-specific.
	IsSrcDstAssignmentKnown *bool `json:"is_src_dst_assignment_known,omitempty" parquet:"is_src_dst_assignment_known,optional"`
	...
	// Observation Point: Indicates whether the source network endpoint, destination network endpoint, or neither served as the observation point for the activity. The value is normalized to the caption of the <code>observation_point_id</code>.
	ObservationPoint *string `json:"observation_point,omitempty" parquet:"observation_point,optional"`

	// Observation Point ID: The normalized identifier of the observation point. The observation point identifier indicates whether the source network endpoint, destination network endpoint, or neither served as the observation point for the activity.
	ObservationPointId *int32 `json:"observation_point_id,omitempty" parquet:"observation_point_id,optional"`

	// Packets: The list of packet objects describing captured network packets.
	PacketList []Packet `json:"packet_list,omitempty" parquet:"packet_list,list,optional"`
	...
	// Source Endpoint:  The initiator of the network connection. In some contexts an event source cannot correctly identify the initiator. Refer to <code>is_src_dst_assignment_known</code> for certainty.
	...
	// Traffic: The network traffic for this observation period. Use when reporting: (1) delta values (bytes/packets transferred since the last observation), (2) instantaneous measurements at a specific point in time, or (3) standalone single-event metrics. This attribute represents a point-in-time measurement or incremental change, not a running total. For accumulated totals across multiple observations or the lifetime of a flow, use <code>cumulative_traffic</code> instead.
	...
	// URL: The URL details relevant to the network traffic.
	Url *UniformResourceLocator `json:"url,omitempty" parquet:"url,optional"`
}

func (v *NetworkActivity) Observable() (*int, string) {
	return nil, ""
}

func (v *NetworkActivity) ValidateObservables() error {
...
var NetworkActivityFields = []arrow.Field{
	...
	{Name: "app_protocol_name", Type: arrow.BinaryTypes.String, Nullable: true},
	...
	{Name: "cumulative_traffic", Type: NetworkTrafficStruct, Nullable: true},
	...
	{Name: "is_src_dst_assignment_known", Type: arrow.FixedWidthTypes.Boolean, Nullable: true},
	...
	{Name: "network_observation_point", Type: NetworkEndpointStruct, Nullable: true},
	{Name: "observables", Type: arrow.ListOf(ObservableStruct), Nullable: true},
	{Name: "observation_point", Type: arrow.BinaryTypes.String, Nullable: true},
	{Name: "observation_point_id", Type: arrow.PrimitiveTypes.Int32, Nullable: true},
	{Name: "packet_list", Type: arrow.ListOf(PacketStruct), Nullable: true},
